...
void BumpTolerantController::BTC(const mrs_msgs::UavState &uav_state,
  ...
  if (wall_interaction_state_ == ALIGNING_TO_WALL) {
    ...
    if (first_alignment_iteration_ && contact_point_msg) {
      ...
      recovery_position_ = uav_pos_W - direction_to_contact * 0.3; // 30cm
    ...
    // --- PHASE 2: MOVE AWAY FROM CONTACT ---
      ...
      bool mooving_towards_target = std::sqrt(uav_vel_B.x() * uav_vel_B.x() + uav_vel_B.y() * uav_vel_B.y()) > 0.1 ||
                                    std::abs(uav_pos_W.head<2>().norm() - recovery_position_.head<2>().norm()) < 0.25;
      ...
      if (!mooving_towards_target) {
        ...
        if (stuck_counter_ > 100) {
          ...
          recovery_position_       = uav_pos_W - direction_to_recovery * 0.3;
          stuck_counter_           = 0;
        }
      ...
      if (pos_error.head<2>().norm() < 0.1 && std::sqrt(uav_vel_B.x() * uav_vel_B.x() + uav_vel_B.y() * uav_vel_B.y()) < 0.3
          && std::abs(yaw_error) < 0.05) { // ~3 degrees threshold
    ...
    bool mooving = std::abs((R_OrientAdjust * uav_vel_B).y()) > 0.5 * contact_velocity_magnitude;
    ...
    ROS_INFO_THROTTLE(0.1, "[%s][SLIDING_SQUARE]: Mooving: %s, contact velocity magnitude: %.3f/%.3f, current velocity: [%.3f, %.3f, %.3f], %.3f>%.3f, q_wall: %.3f",
              ...
              uav_pos_W.y(), uav_pos_W.x() * m_wall + q_wall,
              q_wall);

    bool mooving_forward = uav_vel_W.y() > 0.0;
    bool mooving_backward = !mooving_forward;

    bool ahead_of_wall_with_margin = uav_pos_W.y() > uav_pos_W.x() * m_wall + q_wall + 2.0 && ready_to_sliding_ && mooving ;
    bool behind_wall_with_margin = uav_pos_W.y() < uav_pos_W.x() * m_wall + q_wall - 2.0 && ready_to_sliding_ && mooving ;
    ...
    if (ready_to_sliding_ && wall_estimation_weight > 0.8) {
      ...
      bool time_elapsed = (ros::Time::now() - last_wall_point_update_time_).toSec() > 0.5;

      if ((new_valid_wall_point || time_elapsed) && wall_points.size() >= max_wall_points) {
        ...
        if (std::abs(denominator) > 1e-6) {
          ...
          ROS_INFO_THROTTLE(0.1, "[%s][SLIDING_SQUARE]: Linear regression: m_wall=%.3f, q_wall=%.3f", name_.c_str(), m_wall, q_wall);
        } else {
    ...
    double sliding_velocity_magnitude = std::clamp(sliding_direction_ * 3.0 * yaw_reduction_term * normal_reduction_term,
    ...
    if (std::abs(current_yaw - desired_attitude_slide.getYaw()) > M_PI / 3) { // 60 degrees
